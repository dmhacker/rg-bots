...
class Missions:
    ...
    def update(self, game, p_id):
        ...
        def move(curr, dest, failsafe=True):
            cleanse(curr)
        ...
        def should_suicide(loc):
            en = filter(lambda x : robots[x]['hp'] > 10 and self.enemy_flees_when_hp_low, nearby_enemies(loc, 1))
            if len(en) == 4:
                if robots[loc]['hp'] < 25:
                    return 'yes'
                else:
                ...
                # enemy_might_flee(x) == False
                open_spaces = [os for os in around(loc, False) if os not in filter(lambda x : robots[x]['hp'] > 10 and self.enemy_flees_when_hp_low, enemy_locs)]
                dont_move = []
                for osp in open_spaces:
                    movers = nearby_enemies(osp, 1)
                    if really_stupid(osp) or len(movers) > 0:
                ...
                escapes = filter(lambda x : x not in en and len(nearby_enemies(x, 1)) == 0, around(loc, tillspawn <= 1))
                if len(escapes) == 0:
                    if robots[loc]['hp'] < 20 or (spawn(loc) and tillspawn == 0):
                        return 'yes'
                    else:
        ...
        def hits_to_kill_enemy(enemy_hp):
            local_hp = enemy_hp
            hits = 0
            while local_hp > 0:
                local_hp -= 8
        ...
        actual_ally_locs.sort(key=lambda x : rg.wdist(x, closest_enemy(x)))
        ...
        for bot in actual_ally_locs:
            if bot not in gtfo_bots:
                ...
                def gtfo_remove(loc):
                    for gtfo in gtfo_bots:
                        if gtfo == loc:
                            gtfo_bots.remove(gtfo)
                ...
                if should_suicide(bot) == 'yes':
                    suicide(bot)
                elif should_suicide(bot) == 'guard':
                    guard(bot)
                    gtfo_remove(bot)
                            
        if spawn(bot) and bot not in gtfo_bots:
            if tillspawn == 1:
                final_resorts = [e for e in around(bot, no_spawn=False) if e not in ally_locs and unmovable_spawn(e) == False]
                final_resorts.sort(key=lambda x : danger_value(x))
                if len(final_resorts) > 0:
                    move(bot, final_resorts[0], failsafe=False)
        ...
        for enemy in enemy_locs:
            if enemy in self.attacking_allies.values():
                bae = [b for b in actual_ally_locs if rg.wdist(enemy, b) == 1]
                attackers = [a for a in bae if a in self.attacking_allies and self.attacking_allies[a] == enemy]
                attackers.sort(key=lambda x : robots[x]['hp'], reverse=True)
                actually_attacking = []
                for attacker in attackers:
                    hits = hits_to_kill_enemy(robots[enemy]['hp'])
                    if hits >= len(actually_attacking) + 1:
                        attack(attacker, enemy)
                        actually_attacking.append(attacker)
                    else:
                        guard(attacker)
